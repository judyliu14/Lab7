...
router.setState = function (state, entryNum) {
  ...
  // state will be 0 if settings page
  if (entryNum > 0) {
    ...
    let newElement = document.createElement("entry-page");
    let oldElement = document.querySelector("entry-page");
    document.querySelector("body").removeChild(oldElement);
    document.querySelector("body").appendChild(newElement);
    newElement.entry = entryPage.entry;
  } else if (entryNum === 0) {
    document.body.setAttribute("class", "settings");
    history.pushState({ page: entryNum }, "entry" + entryNum, "#settings");
    document.body.querySelector("h1").textContent = "Settings";
  } else {
    history.back();
  }
